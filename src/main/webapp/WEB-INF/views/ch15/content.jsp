	...
	<div class="card-body">
		...
		<div class="card">
			...
			<script>
				function boardList() {
					$.ajax({
						url: "boardList" //요청 url
					}).done((data) => {
						if(data.result === "fail") { //json으로 넘어와야 이렇게 나옴.
							window.location.href = "${pageContext.request.contextPath}/ch14/login";		//자바스크립트 코드에서 화면 이동->controller url
						} else {
							$("#boardList").html(data); //html 조각
						}
					});
				}
			</script>
