...
public class Ch15Controller {
   ...
   public String afterThrowing() {
      ...
      boolean result = true;
      if(result) {
         throw new RuntimeException("테스트 예외입니다."); //무조건 예외 발생
      }
      return "redirect:/ch15/content";
   }
   ...
   public String runtimeCheck() {
      log.info("실행");
      Pager pager = new Pager(10, 5, boardService.getTotalBoardNum(), 1); //게시물 목록 가져오기
      List<Ch14Board> boards = boardService.getBoards(pager);
      return "redirect:/ch15/content"; //redirect로 넘어가면, 데이터를 session에 저장해 주어야 한다.
   }
   
   @GetMapping("/boardList")
   @Ch15Aspect8LoginCheck //로그인 체크 기능
   public String boardList(Model model) {
      log.info("실행");
      Pager pager = new Pager(5, 5, boardService.getTotalBoardNum(), 1);
      List<Ch14Board> boards = boardService.getBoards(pager); //게시물 목록을 boardList로 넘어감.
